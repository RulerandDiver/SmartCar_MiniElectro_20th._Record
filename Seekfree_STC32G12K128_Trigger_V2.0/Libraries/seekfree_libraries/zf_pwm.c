...
/*************************************************************************
*  函数名称：void Motor_Ctrl(int16 motor1,int16 motor2)
*  功能说明：测试程序
*  参数说明：无
*  函数返回：无
*  修改时间：2022年3月10日
...
#define  MOTOR_PWM_LIMIT  8000    //PWM输出限幅60%
void Motor_Ctrl(int16 motor1,int16 motor2)
{
    if (motor1 > 0)
   {
		if (motor1 > MOTOR_PWM_LIMIT)
		{
			motor1 = MOTOR_PWM_LIMIT;
		}
		pwm_duty(PWMB_CH2_P75, motor1);
		P41=1;
   ...
   else
   {
		...
		pwm_duty(PWMB_CH2_P75,  (0-motor1));
		P41=0;
   }

   if (motor2 > 0)
   {
		if (motor2 > MOTOR_PWM_LIMIT)
		{
			motor2 = MOTOR_PWM_LIMIT;
		}
		pwm_duty(PWMB_CH3_P76, motor2);
		P42=1;
   }
   else
   {
		...
		pwm_duty(PWMB_CH3_P76,  (0-motor2));
		P42=0;
   }
}
...
void Motor_Init()
{
	pwm_init(PWMB_CH2_P75, MOTOR_PWM_FREQ, 0); 	//初始化PWM1  使用P60引脚  初始化频率为17Khz
	pwm_init(PWMB_CH3_P76, MOTOR_PWM_FREQ, 0); 	//初始化PWM2  使用P62引脚  初始化频率为17Khz
	
	gpio_mode(P4_1, GPO_PP);	// P64引脚设置为推挽输出
	gpio_mode(P4_2, GPO_PP);	// P60引脚设置为推挽输出
}
