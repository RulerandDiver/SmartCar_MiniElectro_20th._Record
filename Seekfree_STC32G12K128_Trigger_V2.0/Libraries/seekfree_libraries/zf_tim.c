...
Encoder_tt EncoderL, EncoderR;

static int I_abs(int num)
{
...
void Encoder_Init()
{
	...
	EncoderL.encoderlast = 0;
	EncoderL.count = 0;
	
	EncoderR.encoderlast = 0;
	EncoderR.count = 0;
}
...
int get_left_encoder(void)
{
	...
	if(LEFT_DIR == 1)
	{
		encoder_left = ctimer_count_read(CTIM4_P06);
	}
	else
	{
		encoder_left = ctimer_count_read(CTIM4_P06) * -1;
	}
...
{
	// 检测异常跳变,当检测到当前值与上次值差异过大(>20)且连续稳定计数达到5次时，认为当前值是干扰
	if (I_abs(encoder - instance->encoderlast) > 20 && instance->count >= 5)
	{
		// 认为是干扰，使用上次值
